...
export class DataService {
    ...
    public async getRequestData(requestId:string) {
        return new Promise<any[]>(async(resolve, reject) => {
            sp.web.lists.getByTitle('Travel Requests').items
                .select("Title", "Status", "Stage", "NextApprover", "RequestLog", "RequestData")
                .orderBy("Title", true)
    ...
    public SaveRequest(st, kickoffValue?) {
        return new Promise<string>(async (resolve, reject) => {
            ...
            let data = {                    
                ...
                RequestDate:JSON.stringify(req.dateOfRequest).replace("\"","").replace("\"",""),
                Status: req.status,
                Stage: req.stage,
            ...
            if(!st.requestID){
                ...
                data['Title'] = title;
                let item = await sp.web.lists.getByTitle("Travel Requests").items.add(data);
                console.log(item);
                itemId = item.data.Id;
        ...
        return new Promise<string>(async (resolve, reject) => {
            ...
            // if(!st.requestID){
                try {
                    // add an item to the list                
                    // data['Title'] = title;
                    let item = await sp.web.lists.getByTitle("PrintRequests").items.add(data);
                    console.log(item);
                    itemId = item.data.Id;
                }
                catch(e){
            ...
            if (formState.files.length > 0) {
                for (let i = 0; i < formState.files.length; i++) {
                    try {
                        ...
                        let tmpFileName = file.name.substring(0, file.name.lastIndexOf("."));
                        let fName = tmpFileName + "_" + unique + ext;
                        let result = await web.getFolderByServerRelativeUrl('FormAttachments').files.add(fName, file, true);
                        ...
                        console.log("Error loading attachment or setting associted metadata");
                        reject(e);
                    }
                }
            }
    ...
    public async GetAttachments(formKey: string){
        return new Promise<any>(async (resolve, reject) => {
            let web = new Web(this.pc.web.absoluteUrl);
            web.lists.getByTitle("Form Attachments").items
                .select("FileLeafRef", "Id")
    ...
    public async RemoveAttachment(attId:string){
        return new Promise<any>(async (resolve, reject) => {
            let web = new Web(this.pc.web.absoluteUrl);
            await web.lists.getByTitle("Form Attachments").items.getById(Number(attId)).delete();
            resolve("success");
        });
    }
    ...
    public async GetApprovers(employee: string){
        return new Promise<any>(async (resolve, reject) => {
            let web = new Web(this.pc.web.absoluteUrl);
            web.lists.getByTitle("Travel Requests - Approval Matrix").items
                .expand("Employee/Name", "Admin","SectionHead","Secretary","Undersecretary","DeputyUndersecretary","Budget","AcctMgr1","AcctMgr2","AcctAdmin")
                .select("Employee/Name", "Admin/Name", "Admin/Id", "Admin/Title","Admin/UserName","AcctAdmin/UserName","AcctAdmin/Id", "AcctAdmin/Title","SectionHead/UserName","SectionHead/Id","SectionHead/Title","Secretary/UserName","Undersecretary/UserName","Undersecretary/Id","DeputyUndersecretary/UserName","DeputyUndersecretary/Id","Secretary/Title","Secretary/Id","Undersecretary/Title","DeputyUndersecretary/Title","Budget/UserName","Budget/Title","Budget/Id","AcctMgr1/UserName","AcctMgr2/UserName","Agency","PersonnelNo")
